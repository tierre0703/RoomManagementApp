...
class Channel {
    ...
    init(port: UInt16, type: ChannelType, router: Router?) {
        ...
        self.type = type
        self.router = router
        LogID = "Channel \(type.rawValue.capitalized)"
    ...
    public func start() {
        do {
            ...
            listener?.newConnectionHandler = { [self] connection in
                Logger.Instance.AddLog(msg: "\n\(LogID) Received new connection")
                setup(connection: connection)
            }
            listener?.start(queue: dispatchQueue)
            
            Logger.Instance.AddLog(msg: "\n\(LogID) Listening on port \(port)")
        } catch {
            Logger.Instance.AddLog(msg: "\n\(LogID) Error creating listener \(error)")
        }
    }
    ...
    private func setup(connection: NWConnection) {
        dispatchQueue.async { [self] in
            ...
            .sink { [self, weak networkSession] user in
                ...
                Logger.Instance.AddLog(msg: "\n\(LogID) Received registration for user \(user.deviceName)")
