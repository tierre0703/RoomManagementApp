...
class Router {
    ...
    private var calls = [PushCall]()
    private let logger = SimplePushKit.Logger(prependString: "Router", subsystem: .general)
    ...
    private func handleCallAction(_ callAction: CallAction, from client: Client) {
        let call: PushCall! = self.call(for: client.user)
        ...
        if call == nil {
            ...
            case .connect:
                ...
                let newCall = PushCall(status: .pending, participants: [client.user, callAction.receiver!])
    ...
    private func cancel(call: PushCall, with action: CallAction.Action, initiatedBy user: User) {
    ...
    private func call(for user: User) -> PushCall? {
        calls.first { call -> Bool in
            call.participants.contains(user)
        }
    }
    
    private func otherParticipant(in call: PushCall, for peer: User) -> User? {
