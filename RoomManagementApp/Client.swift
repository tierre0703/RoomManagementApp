...
class Client {
    ...
    private let logger: SimplePushKit.Logger
    init(user: User) {
        self.user = user
        
        logger = SimplePushKit.Logger(prependString: "\(user.deviceName) - Client", subsystem: .general)
    ...
    func setSession(_ session: NetworkSession, type: Channel.ChannelType) {
        dispatchQueue.async { [weak self] in
            ...
            session.logger = SimplePushKit.Logger(prependString: "\(self.user.deviceName) - \(type.rawValue.capitalized) Channel - NetworkSession", subsystem: .networking)
            var heartbeatCoordinator: HeartbeatCoordinator?
            
            if let session = session as? RequestResponseSession {
                let heartbeatLogger = SimplePushKit.Logger(prependString: "\(self.user.deviceName) - \(type.rawValue.capitalized) Channel - Heartbeat", subsystem: .heartbeat)
